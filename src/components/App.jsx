...
function App() {
  ...
  function handleCompletedItems() {
    ...
    const completedItems = items.filter((item, id) =>
  ...
  function handleDeleteSelected() {
    ...
    const updatedItems = items.filter(
      (item, id) => !selectedItems.includes(id)
    );
  ...
  return (
    <div
      className="motherdiv"
      style={{
        color: listColor ? "hsl(233, 11%, 84%)" : " hsl(235, 21%, 11%)",
      }}
    >
      <div className="todolist">
        {/* ---------------------- CreateList.jsx-------------------- */}
        <CreateList onAdd={addNote} />
        <div
          className="boxshadow"
          style={{
            boxShadow: listColor
              ? "1px 1px 5px hsl(235, 24%, 19%)"
              : "1px 1px 5px hsl(236, 33%, 92%)",
          }}
        >
          <div
            className="total"
